...
class EquivariantVAE(torch.nn.Module):
    """
    ...
    Decoder  : z          -->  v_raw   
                          -->  c_recon 
    dec_out   : hidden_irreps --> out_irreps 
                Produces v_raw, the flat steerable vector fed to EquivariantDecoder.
    dec_recon : hidden_irreps --> input_irreps
                Produces c_recon in the same SH-coefficient space as the VAE input

    Loss = L_zernike + L_ipt_sh + beta * L_kl  
    """
    ...
    def __init__(
        ...
        input_parts = [f"{R}x{l}{'e' if l%2==0 else 'o'}" for l in range(l_max + 1)]
        self.input_irreps = Irreps(" + ".join(input_parts))

        hidden_parts = [
            f"{'64' if l <= 2 else '32'}x{l}{'e' if l%2==0 else 'o'}"
            for l in range(l_max + 1)
        ]
        self.hidden_irreps = Irreps(" + ".join(hidden_parts))

        latent_parts = [
            f"{latent_channels}x{l}{'e' if l%2==0 else 'o'}"
            for l in range(l_max + 1)
        ]
        ...
        self.dec_recon = E3Linear(self.hidden_irreps,  self.input_irreps)
    ...
    def _expand_scalars(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for l in range(self.l_max + 1):
            m = 2 * l + 1
            expanded.append(
                x[:, idx : idx + self.latent_channels].repeat_interleave(m, dim=1)
            )
    ...
    def _e3nn_to_c(self, flat: torch.Tensor, mul: int) -> torch.Tensor:
        B = flat.shape[0]
        segments = []
        sh_idx = 0
        for l in range(self.l_max + 1):
            m          = 2 * l + 1
            block_size = mul * m
            seg        = flat[:, sh_idx : sh_idx + block_size]
            seg        = seg.reshape(B, mul, m).permute(0, 2, 1)  
            segments.append(seg)
            sh_idx += block_size
        return torch.cat(segments, dim=1)   
    ...
    def forward(self, c_target: torch.Tensor):
        ...
        c_recon_flat = self.dec_recon(h)
        c_recon      = self._e3nn_to_c(c_recon_flat, mul=self.R)   

        return v_raw, c_recon, mu, logvar_expanded
