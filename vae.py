...
#     Encoder compresses c_ipt_sh into a steerable latent z via the
#     reparameterisation trick. Decoder expands z into v_raw, which is
#     passed to the EquivariantDecoder to produce the Zernike prediction.

#     Loss = L_zernike + beta * L_kl
#     The IPT-SH reconstruction branch has been removed — the KL term
#     alone is sufficient to regularise the latent space.
#     """
...
#         # logvar: one scalar per (l, channel) — broadcast to full latent dim
...
#         """
#         Broadcast scalar logvar (B, latent_channels*(l_max+1)) to the full
#         latent dimension by repeating each scalar across the 2l+1 components
#         of its l-block.
#         """
#         expanded = []
#         idx = 0
...
#         if self.training:
#             std = (0.5 * logvar_expanded.clamp(-10, 10)).exp()
...
#         mu     = self.enc_mu(x)
#         # Clamp mu norm to a fixed radius to prevent KL explosion.
#         # This keeps the posterior mean close to the prior without
#         # relying on weight initialisation tricks that don't work with e3nn.
#         mu = mu / (mu.norm(dim=-1, keepdim=True).clamp(min=1e-8) + 1e-8) * 3.0

#         logvar = self.enc_logvar(x)xs
...
#         return v_raw, mu, logvar_expanded
...
class EquivariantVAE(nn.Module):
    ...
    def __init__(
        ...
        self.enc_mu     = E3Linear(self.hidden_irreps, self.latent_irreps)
        self.enc_logvar = E3Linear(self.hidden_irreps, self.logvar_irreps)
        torch.nn.init.zeros_(self.enc_logvar.weight)
        ...
        # Reconstruction branch removed: pipeline no longer uses c_vae_out.
