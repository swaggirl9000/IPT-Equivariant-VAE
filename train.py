...
from get_directions import get_directions
from get_mnist import PointCloudMNIST
# from get_pointcloud import PointCloudShapeNet
from pipeline import IPTVAEPipeline, compute_loss
...
def train(config: dict):
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            c_pred, c_zernike, c_ipt_sh, c_vae_out, mu, logvar_expanded = model(pc)

            losses = compute_loss(
                c_pred=c_pred, c_zernike=c_zernike,
                c_ipt_sh=c_ipt_sh, c_vae_out=c_vae_out,
                mu=mu, logvar_expanded=logvar_expanded,
                l_max=config["l_max"],
                beta=beta,
